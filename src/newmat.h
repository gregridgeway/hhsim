...
class RowSequence : public GeneralMatrix
{
public:
   RowSequence(Real first, Real last, Real step);
   RowSequence(Real first, Real last);
   RowSequence(int n, Real first, Real step, bool dummy);
   void operator=(const RowSequence& seq);
   void operator+=(const RowSequence&);
   void operator-=(const RowSequence&);
   void operator*=(const RowSequence&);
   void operator+=(Real);
   void operator-=(Real r) { operator+=(-r); }
   void operator*=(Real);
   void operator/=(Real r) { operator*=(1.0/r); }
   void GetRow(MatrixRowCol&);                       // Get matrix row
   void NextRow(MatrixRowCol&);                      // Go to next row
   void GetCol(MatrixRowCol&);                       // Get matrix col
   void GetCol(MatrixColX&);                         // Get matrix col
   void NextCol(MatrixRowCol&);                      // Go to next col
   void NextCol(MatrixColX&);                        // Go to next col
   MatrixType Type() const;
   friend bool operator==(const RowSequence s1, const RowSequence s2);
   friend RowSequence operator+(const RowSequence s1, const RowSequence s2);
   friend RowSequence operator-(const RowSequence s1, const RowSequence s2);
   friend RowSequence operator+(const RowSequence s, Real r)
      { return RowSequence(s.first+r,s.last+r,s.step); }
   friend RowSequence operator+(Real r, const RowSequence s)
      { return RowSequence(s.first+r,s.last+r,s.step); }
   friend RowSequence operator-(const RowSequence s, Real r)
      { return RowSequence(s.first-r,s.last-r,s.step); }
   friend RowSequence operator-(Real r, const RowSequence s)
      { return RowSequence(s.first-r,s.last-r,s.step); }
   friend RowSequence operator*(const RowSequence s, Real r)
      { return RowSequence(s.first*r,s.last*r,s.step*r); }
   friend RowSequence operator*(Real r, const RowSequence s)
      { return RowSequence(s.first*r,s.last*r,s.step*r); }
   friend RowSequence operator/(const RowSequence s, Real r)
      { return RowSequence(s.first/r,s.last/r,s.step/r); }
